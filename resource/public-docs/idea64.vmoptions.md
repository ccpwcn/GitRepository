...
# 默认开启回收线程数，和CPU数量一样就好了
-XX:ParallelGCThreads=8

# 年轻代使用并行回收器，以关注响应时间为核心目标
-XX:+UseParNewGC
...
# 在这种模式下，JIT编译器停用、字节码将不会被编译为机器码，虽然应用程序会继续运行，但是运行速度和性能会大幅度降低。
# 结果就是：设置较大的代码缓存或者提高代码的缓存大小，将会延缓代码缓存区域发生内存溢出。
# 使用-XX:+UseCodeCacheFlushing 这个参数，我们可以避免当代码缓存被填满的时候JVM切换到interpreted-only 模式。
